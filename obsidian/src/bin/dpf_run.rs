...
fn main() {
    ...
    // FIRST FSS: Bid encoding over domain [domain_size]
    ...
    // Add this debug section before the tie detection loop
    // println!("\n=== DEBUG: Column sums ===");
    // for idx in 0..domain_size { // Print first 10 for debugging
    //     let col_sum_0_val = all_client_s0[idx].clone();
    //     let col_sum_1_val = all_client_s1[idx].clone();
    //     let col_sum_total = (col_sum_0_val + col_sum_1_val); // Avoid overflow
    //     println!("Bid level {}: col_sum = {}", idx, col_sum_total.value());
    // }
    ...
    while current_threshold > 0 && !second_highest_found {
        ...
        // println!("check {}", exact_one_check.value());
